...
parser.add_argument('--lib', help = 'library name', type = str, default = '')
        ...
        for idx, (l1, l2) in enumerate(zip(f1, f2)):
            ...
            if l == 3 and len(l1) > 0:
                ...
                try:
                    ...
                    if len(lib) > 0:
                        name = ';'.join([n1] + [':'.join(x) for x in zip(['LY','SS','CB','QT','RX','RQ','SM'], [lib, cellbcseq, originalBC, cellbcphred, umiseq, umiphred, str(cellID).zfill(3)])])
                    else:
